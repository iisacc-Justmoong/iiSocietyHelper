...
const QString message = iiSocietyHelper::helloWorld(); // 정확히 "Hello world!"
```

공개 헤더와 구현은 저장소 루트의 `iiSocietyHelper.h`, `iiSocietyHelper.cpp`에 함께 배치한다. `IISOCIETYHELPER_EXPORT`가 심볼을 내보내며 `IISOCIETYHELPER_BUILDING_LIBRARY`는 라이브러리 빌드에만 정의한다.
...
스크립트는 고정된 `build/`에서 Release 구성과 빌드, CTest를 수행한 다음 `$HOME/.local/SDK/iiSocietyHelper`에 설치한다. 이어서 `tests/consumer`를 `build/consumer/build/`에 독립 구성하고, 설치된 CMake 패키지와 헤더 및 동적 라이브러리만 사용하여 빌드하고 실행한다. 테스트는 반환 문자열, C++20 사용 여부, Qt 컴파일 버전 및 런타임 버전 6.8.3을 검증한다.
...
INSTALL_PREFIX="$HOME/.local/SDK/iiSocietyHelper" \
...
기본 설치 경로 아래에 `include/iiSocietyHelper.h`, `lib/`의 공유 라이브러리, `lib/cmake/iiSocietyHelper/`의 Config·ConfigVersion·Targets, `share/iiSocietyHelper/README.md`가 생성된다. Windows 런타임 DLL은 `bin/`에 설치된다. 기본 설치 경로는 이 SDK가 최상위 CMake 프로젝트일 때만 적용한다. 직접 CMake로 구성할 때 HOME이 없으면 USERPROFILE을 기본 설치 경로 기준으로 사용한다.

```cmake
find_package(iiSocietyHelper 0.1.0 CONFIG REQUIRED)
...
iiSocietyHelper의 자체 작성 코드와 문서는 GNU Affero General Public License v3.0 전용으로
