...
install_prefix="${INSTALL_PREFIX:-$HOME/.local/SDK/iiSocietyHelper}"
...
cmake -S "$source_dir/tests/consumer" -B "$consumer_build_dir"     -DCMAKE_BUILD_TYPE=Release     "-DiiSocietyHelper_DIR=$install_prefix/lib/cmake/iiSocietyHelper"     "-DCMAKE_PREFIX_PATH=$install_prefix${search_prefixes:+;$search_prefixes}"
